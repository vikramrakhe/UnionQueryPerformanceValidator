...
namespace UnionQueryPerformanceValidator
{
    public class StringQueryStore
    {
        ...
        {
            "select PUID from pitem where rownum < 100000",
            "select PUID from pitemrevision where rownum< 100000",
            "select PUID from pworkspaceobject where rownum < 100000",
            "select PUID from ppom_application_object where rownum< 100000",
            "select PUID from PImanRelation where rownum < 100000"
        };
